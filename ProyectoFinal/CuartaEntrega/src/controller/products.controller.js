...
export const getProductos = async (req,res) => {
    const {limit,page,categoria,sort} = req.query;
    const cat = categoria ? categoria : ["Categoria 1", "Categoria 2", "Categoria 3", "Categoria 4"];
    const pag = page ? page : 1;
    const lim = limit ? limit : 10;
    const ord = sort == "asc" ? 1 : -1;

    try {
        const prods = await productoModel.paginate({categoria: cat},{limit: lim, page: pag, sort:{precio:ord}});
