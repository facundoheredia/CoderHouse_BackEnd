...
export const postCompra = async (req,res) => {
    ...
    try {
        const carrito = await carritoModel.findById(cid);
        if (carrito) {
            const productosVerificados = verificarStock(carrito.productos);
            const montoTotalCompra = calcularMontoCompra(productosVerificados);
            const nuevoTicket = await ticketModel.create({montoCompra: montoTotalCompra,productos: productosVerificados});
...
const verificarStock = async (productosEnCarrito) => {
    ...
    for (let productoEnCarrito of productosEnCarrito) {
        let productoEnDeposito = await productoModel.findById(productoEnCarrito.id);

        if(productoEnDeposito) {
            if(productoEnCarrito.cantidad <= productoEnDeposito) {
